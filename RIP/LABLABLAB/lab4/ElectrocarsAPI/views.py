...
from ElectrocarsAPI.permissions import *
...
class StationsList(APIView):
    ...
    def get(self, request, format=None):
        ...
        if(user):
            currentReport = Power_report.objects.filter(status='Draft', creator_id = user.id).first()
        else:
            currentReport = None
        if(station_name):
...
@swagger_auto_schema(manual_parameters=[status_param,start_date_param,end_date_param],
                     method="Get",responses={200:Power_reportsSerializer(many=True)})
@api_view(['Get'])
@permission_classes([IsAuthenticated])
def get_reports(request, format=None):
    ...
    if(user.is_staff or user.is_superuser):
        ...
        reports = Power_report.objects.exclude(status='Draft').exclude(status="Deleted").filter(creator_id = user)
    if(filter_status):
...
power_schema = openapi.Schema(title="power",type=openapi.TYPE_INTEGER,description="Новая температура")
@swagger_auto_schema(method="Put",request_body=power_schema,responses={200:"Power added",400:"report is not in draft"})
@api_view(['Put'])
@permission_classes([IsAuthenticated])
